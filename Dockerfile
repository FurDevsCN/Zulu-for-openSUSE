...
RUN zypper --non-interactive in `curl -sX GET "https://api.azul.com/metadata/v1/zulu/packages/?java_version=8&os=linux&arch=amd64&archive_type=rpm&java_package_type=jre&support_term=lts&latest=true&availability_types=CA&include_fields=&page=1&page_size=2" | jq '.[] | .download_url' |sed 's/\"//g'` || true
# 同上，获取zulu jre 11版本的rpm下载链接，并下载。
RUN zypper  --non-interactive in `curl -sX GET "https://api.azul.com/metadata/v1/zulu/packages/?java_version=11&os=linux&arch=amd64&archive_type=rpm&java_package_type=jre&support_term=lts&latest=true&availability_types=CA&include_fields=&page=1&page_size=2" -H "accept: application/json" | jq '.[] | .download_url' |sed 's/\"//g'` || true
...
# 卸载之前安装的工具。
RUN zypper --non-interactive rm jq curl wget gzip
# 应用系统更新补丁
RUN zypper patch
# 验证系统完整性。
RUN zypper ve
